...
namespace Fileharbor.Services
{
    ...
    public class ContentTypeService : ServiceBase, IContentTypeService
    {
        ...
        public ContentTypeService(ILogger<ContentTypeService> logger, IPermissionService permissionService, IDbConnection database)
            : base(database)
        {
            _logger = logger;
        ...
        public async Task<bool> HasColumnMappingAsync(Guid collectionId, Guid contentTypeId, Guid columnId, Transaction transaction)
        {
            var database = await GetDatabaseConnectionAsync();
            transaction = transaction.Spawn(database);
        ...
        public async Task AddColumnMappingAsync(Guid collectionId, Guid contentTypeId, ColumnMapping mapping, Transaction transaction)
        {
            var database = await GetDatabaseConnectionAsync();
            transaction = transaction.Spawn(database);
            ...
            try
            {
                _logger.LogDebug(LoggingEvents.InsertItem, "Starting creation of new column mapping for content type {0} in collection {1}", contentTypeId, collectionId);
                ...
                _logger.LogDebug(LoggingEvents.InsertItem, "Finished creation of new column mapping in collection {0} with id {1}", collectionId, mapping.Id);
            }
            catch (Exception e)
            {
                _logger.LogWarning(LoggingEvents.InsertItem, e, "Unable to create column mapping - unexpected exception");
        ...
        public async Task CreateContentTypeAsync(Guid collectionId, ContentType contentType, Transaction transaction)
        {
            ...
            try
            {
                _logger.LogDebug(LoggingEvents.InsertItem, "Starting creation of new contentType in collection {0} with id {1}", collectionId, contentType.Id);
                ...
                _logger.LogDebug(LoggingEvents.InsertItem, "Finished creation of new contentType in collection {0} with id {1}", collectionId, contentType.Id);
            }
            catch (Exception e)
            {
